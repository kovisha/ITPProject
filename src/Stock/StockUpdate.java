...
public class StockUpdate extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int maxUnitsSold = Integer.parseInt(request.getParameter("maxUnits"));
		int leadTime = Integer.parseInt(request.getParameter("leadTime"));
		int AverageUnitsSold = Integer.parseInt(request.getParameter("avgUnits"));
		int avgleadTime = Integer.parseInt(request.getParameter("avgleadTime"));
		...
		//declaring the boolean type variable to store the return type from the dao
		  ...
		  HttpSession session = request.getSession();
			...
			isTrue = stockDao.updateStock( id ,stockName,qtySupplied+qtyAvailable-storeQty,stockSize,qtySupplied,suppliedDate,storeQty,supplierName,categoryName,maxUnitsSold,AverageUnitsSold,leadTime,avgleadTime);
