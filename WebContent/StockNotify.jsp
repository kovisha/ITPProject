...
<div class="formHeading">
	<center>
		<h4 >Ensure Stock Re-shelfing before the deadline </h2>
	</center>
</div>
<br>

<form action="viewNotiications" method="post"> 
<center><button type="submit" class="btn btn-primary btn-lg "><i class="fas fa-eye"> </i>View Notifications</button></center>
<!--  <input type="submit"  value="Click here to view the Categories" >-->
 </form> 
 <br>
 ...
 <table class="table table table-hover table-dark table-bordered table-sm text-center table-stripped" id="myTable">
                        <thead>
                        	...
                        	<c:forEach var="notify" items="${listNotifications}">
								
							<c:set var = "id" value = "${notify.id}"/>
      						<c:set var = "stockName" value = "${notify.stockName}"/>
      						<c:set var = "supplierName" value = "${notify.supplierName}"/>
      						<c:set var = "reOrderLevel" value = "${notify.reOrderLevel}"/>
      						<c:set var = "stockSize" value = "${notify.stockSize}"/>
      						<c:set var = "reshelfDate" value = "${notify.reshelfDate}"/>
      						<c:set var = "reshelfQty" value = "${notify.reshelfQty}"/>
      						
      						<c:url value = "NotificationsDelete.jsp" var = "notifydelete">
								<c:param name="id" value = "${id}"/>
								<c:param name="stockName" value = "${stockName}"/>
								<c:param name="supplierName" value = "${supplierName}"/>
								<c:param name="reOrderLevel" value = "${reOrderLevel}"/>
								<c:param name="stockSize" value = "${stockSize}"/>
								<c:param name="reshelfDate" value = "${reshelfDate}"/>
								<c:param name="reshelfQty" value = "${reshelfQty}"/>
