...
<html>
	...
	<body>
					...
					<%if(qOut.equals("successcusInsertDelDetails")){ %>
						<script>
							Swal.fire({
								title: 'Delivery Details Updated Successfully!!!',
								icon: 'success',
							}).then(function() {
								window.location = "userPaySummary.jsp.jsp";
								});
						</script>
					<%} %>
					<%if(qOut.equals("failedcusInsertDelDetails")){ %>
						<script>
							Swal.fire({
								title: 'Delivery Details Failed Successfully!!!!!!',
								icon: 'error',
							}).then(function() {
